    ...
    def __call__(self, shard_file: str, output_filename: str):
        subprocess.run(
            [
                ...
                "-u",
                "-m",
                "bytelatent.preprocess.preprocess_slm",
                str(shard_file),
                str(output_filename),
            ],
            check=True,
...
def main(
    ...
    for file_batch in chunk(shard_files, 1):  # Process one file at a time
        n_batches += 1
        for shard_file in file_batch:
            output_filename = Path(output_dir) / f"{shard_file.name}.arrow"
            complete_output_filename = Path(output_dir) / f"{shard_file.name}.arrow.complete"
            
            if complete_output_filename.exists():
                n_skipped += 1
            else:
                # Process the job sequentially without Slurm
                job = PreprocessEntropiesJob()
                job(str(shard_file), str(output_filename))  # Assuming the job is callable
                n_launched += 1

    print("launched jobs n=", n_launched)
    print("skipped (completed) jobs n=", n_skipped)
